...
const MenuProductModal = ({ product, open, onClose, onAdd }) => {
  const [qty, setQty] = useState(1);
  const [selectedOptions, setSelectedOptions] = useState({});
  const optionsSchema = useMemo(() => normalizeOptionsSchema(product?.options_schema || product?.optionsschema), [product]);
                ...
                {Object.entries(optionsSchema).map(([key, val]) => (
                  <div key={key} className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      {val.required && <span className="text-sm text-red-600 font-semibold">Required</span>}
                    </div>
